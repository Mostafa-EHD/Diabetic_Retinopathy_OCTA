...
### 设置参数
images_file = '../TASK3_IMAGE'  # 训练图像路径
...
trainval_patient,trainval_label,test_patient,test_label = data_split_crossval(label_file = '../task3_data_fold.xls',fold= 3,aff_info = True)

dataframe_test = pd.read_csv('Task3_ensemble_pseudo_v1.csv')
pseudo_patient = dataframe_test['case'].tolist()
pseudo_label = dataframe_test['class'].tolist()


trainval_patient = trainval_patient + test_patient + pseudo_patient
trainval_label = trainval_label + test_label + pseudo_label



# 训练/验证数据集划分
val_ratio = 0.1  # 训练/验证图像划分比例
train_patient,val_patient,train_label,val_label = train_test_split(trainval_patient,trainval_label, test_size = val_ratio)

train_patient = trainval_patient
val_patient = trainval_patient

train_label = trainval_label
val_label = trainval_label
...
model = timm.create_model('efficientnet_b3', pretrained=True, num_classes=3, in_chans=1)
model_path =  '../Task3_pseudo_v7/weights/model300_1.0.pth'
